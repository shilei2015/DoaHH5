 ...
 * 2. basicSsl 启用后本地 dev 走 HTTP/2，req.headers 会带 `:path` 等伪头，
 *    http-proxy-middleware 转发到 HTTP/1.1 目标会抛异常，且会把整个 dev
 *    进程拖挂。
 * 这里用 Node 原生 fetch 简单做一层转发，既支持 router 语义也不受 HTTP/2 伪头影响。
 */
